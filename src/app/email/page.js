...
export default function EmailInbox() {
  ...
  const handleAIReply = async (id) => {
    ...
    try {
      const response = await fetch(
        `${base_url}/api/email/inbox/${id}/generate-reply`,
        {
          method: "GET",
  ...
  const handleDownloadAttachment = async (attachmentId) => {
    try {
      const response = await fetch(
        `${base_url}/api/email/attachments/${attachmentId}`
      );
      ...
      link.href = url;
      link.download = attachmentId; // Replace with actual filename if available
  ...
  const handleGenerateAIReply = async () => {
    ...
    try {
      const response = await fetch(
        `${base_url}/api/email/inbox/${selectedEmail.id}/generate-reply`
      );
      if (!response.ok) throw new Error("Failed to generate AI reply");
  ...
  const handleSendReply = async () => {
    ...
    try {
      const response = await fetch(
        `${base_url}/api/email/inbox/${selectedEmail.id}/reply`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="flex flex-col bg-black min-h-screen text-light p-6">
      <header className="text-center py-10 w-full max-w-3xl mx-auto">
        <h1 className="text-neonPink text-4xl sm:text-5xl font-extrabold">
          Email Management
        </h1>
      ...
      {linked ? (
        <section className="w-full max-w-full sm:max-w-5xl bg-gray-800 p-6 rounded-lg shadow-lg mx-auto">
          <h2 className="text-neonBlue text-2xl sm:text-3xl font-bold mb-4">
            Your Emails
          </h2>
          ...
          {selectedEmail ? (
            ...
            <div className="p-4 sm:p-6 bg-gray-900 rounded-lg">
              ...
              <h2 className="text-neonPink text-2xl sm:text-3xl font-bold my-2">
                ...
                <div className="mt-4 p-4 bg-gray-300 rounded-lg">
                  ...
                  <ul>
                    {selectedEmail.attachments.map((attachment, index) => (
                      <li
                        key={index}
                        className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 rounded-lg shadow-md hover:bg-gray-200 transition"
                      >
              ...
              <div className="mt-6 flex flex-col gap-4">
                ...
                {aiReply && (
                  <div className="mt-4 w-full">
                    <h3 className="text-neonBlue font-bold mb-2">Edit Reply</h3>
                    <textarea
                      value={userReply}
                      onChange={(e) => setUserReply(e.target.value)}
                      className="w-full sm:max-w-2xl p-2 bg-gray-800 text-white rounded-lg"
                      rows={8}
                    />
                  </div>
                )}
